...
c **********************************************************************
      ...
      tmp='copy/y C:\BERN52\AUTO\T97_MACD.CRD '//apath(1:ii+6)//'\'//fod(11)
      call system(tmp)
      tmp='copy/y C:\BERN52\AUTO\FIXSTA.SIG '//apath(1:ii+6)//'\'//fod(11)
      call system(tmp)
      !! create "RINEX.CRD" ********************************************
      inf=apath(1:ii+6)//'\'//fod(11)//'\RINEX.CRD'
      open(10,file=inf)
      write(10,*)'COORDINATE FILE CREATED BY RXOBV3'
      write(10,*)'--------------------------------------------------------------------------------'
      write(10,*)'LOCAL GEODETIC DATUM: ITRF2008          EPOCH:'
      write(10,*)
      write(10,*)'NUM  STATION NAME           X (M)          Y (M)    Z (M)     FLAG'
      write(10,*)
      close(10)
      !! create "SESSIONS.SES" *****************************************
...
CC     +//' '//dyc//'1 '//apath
      if(ty==2) exe1='PERL C:\GPSUSER52\SCRIPT\BERN52_S30A_MACD.PL '//yyc//' '//dyc//'1 '//apath
      if(ty==2) exe2='PERL C:\GPSUSER52\SCRIPT\BERN52_S30B_MACD.PL '//yyc//' '//dyc//'1 '//apath
...
c **********************************************************************
      ...
      do i=1,nnf
        tmp='copy/y '//opath(1:iii-1)//fixsta(i)//dyc//'1.'//y1c//'o '//  apath(1:ii+6)//'\'//fod(9)
        call system(tmp)
      end do
      !! copy sp3 file to folder "ORB" *********************************
      day_plus=(yy-1995)*365+doy+int((yy-1993)/4)-1
      ...
      close(10)
      !! create "FIXSTA.SIG" *******************************************
      tmp='for %f in ('//apath(1:ii+6)//'\'//fod(11)//'\*.??o) do echo %f >> auto52.inp'
      call system(tmp)
      n=0
      stat=0
      open(10,file='C:\BERN52\AUTO\auto52.inp')
      do i=1,nf
      ...
      open(10,file=inf)
      write(10,*)'Station sigma file           25-OCT-03 15:46'
      write(10,*)'--------------------------------------------------------------------------------'
      write(10,*)
      ...
      do i=1,nnf
        ...
        if(ty==0) then
          do j=1,n
            if(fixsta(i)==osta(j)) then
              write(10,'(a4,20x,"0.0001    0.0001    0.0001")')fixsta(i)
              ty=1
              tyy=1
              exit
            end if
        ...
        if(tyy==1) cycle
        write(10,'(a4,20x,"9.9999    9.9999    9.9999")')fixsta(i)
      end do
      close(10)
